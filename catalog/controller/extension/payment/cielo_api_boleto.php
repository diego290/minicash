...
class ControllerExtensionPaymentCieloApiBoleto extends Controller {
    const EXTENSION = 'payment_cielo_api_boleto';

    public function index() {
        $data = $this->load->language('extension/payment/cielo_api_boleto');
        ...
        $data['alerta'] = '';
        if (isset($this->session->data['cielo_api_boleto_erro'])) {
            $data['alerta'] = $this->session->data['cielo_api_boleto_erro'];
        }

        $tema = $this->config->get(self::EXTENSION . '_tema');

        return $this->load->view('extension/payment/cielo_api/boleto_'. $tema, $data);
    }
    ...
    public function transacao() {
        ...
        if (isset($this->session->data['order_id']) && $this->session->data['payment_method']['code'] == 'cielo_api_boleto') {
            ...
            if (empty($erros_cadastro)) {
                ...
                $new_date = "86400" * $this->config->get(self::EXTENSION . '_vencimento') + mktime(0,0,0,$split_date[1],$split_date[2],$split_date[0]);
                $vencimento = date("Y-m-d", $new_date);
                ...
                $dados['Demonstrative'] = $this->config->get(self::EXTENSION . '_demonstrativo');
                $dados['Instructions'] = $this->config->get(self::EXTENSION . '_instrucoes');
                ...
                if ($resposta) {
                    if (isset($resposta->Payment->Url)) {
                        ...
                        if (isset($this->session->data['cielo_api_boleto_instrucoes'])) {
                            unset($this->session->data['cielo_api_boleto_instrucoes']);
                        }
                        $this->session->data['cielo_api_boleto_instrucoes'] = sprintf($this->language->get('text_instrucoes'), $boleto_vencimento, $boleto_linha, $boleto_url);

                        $this->model_extension_payment_cielo_api_boleto->addTransaction($campos);

                        $this->model_checkout_order->addOrderHistory($order_id, $this->config->get(self::EXTENSION . '_situacao_gerado_id'), $comment, true);
        ...
        if (isset($json['error']) && !empty($json['error'])) { $this->session->data['cielo_api_boleto_erro'] = $json['error']; }
    ...
    private function atualizar_pedido() {
        ...
        $this->load->model('extension/payment/cielo_api_boleto');
        $this->model_extension_payment_cielo_api_boleto->editOrder($order_data, $this->session->data['order_id']);
    }
